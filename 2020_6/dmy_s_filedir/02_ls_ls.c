...
int main(int argc,char* argv[])
{
	...
	char timebuf[20] = {0};
	sprintf(timebuf,"%d month  %d %02d:%02d",filetm->tm_mon + 1,filetm->tm_mday,filetm->tm_hour,filetm->tm_min);
	
	printf("%s %ld %s %s %d %s %s\n",stmode,sb.st_nlink,getpwuid(sb.st_uid)->pw_name,getgrgid(sb.st_gid)->gr_name,sb.st_size,timebuf,argv[1]);	

	printf("-rwxrwxr-x 2 root root 22 6 02 15:35 hello\n");
	return 0;
}
